...
class LINE:
    ...
    def _gen_sampling_table(self):
        ...
        total_sum = sum([math.pow(node_degree[i],power)] for i in range(numNodes)) # 顶点的出度总和
        norm_prob = [float(math.pow(node_degree[j], power)) /
                     total_sum for j in range(numNodes)]
        self.node_accpet, self.node_alias = create_alias_table(norm_prob)

        # create sampling table for edge边采样    numEdges = self.graph.number_of_edges()
    ...
    def batch_iter(self, node2idex):
        edges = [(node2idex[x[0]], node2idex[x[1]]) for x in self.graph.edges()]

        data_size = self.graph.number_of_edges()
        shuffle_indices = np.random.permutation(np.arange(data_size))  # permutation返回一个随机排列的序列
        # postive or negative mod
        mod = 0
        ...
        while True:
            if mod==0:
                ...
                for i in range(len(h)):
                    t.append(alias_sample(self.node_accpet,self.node_alias))
    ...
    def get_embedings(self):   #():元组 []：list列表 {}:字典
        self._embeddings = {}
        if self.order == 'first':
        ...
        for i, embedding in enumerate(embeddings):  # enumerate 把列表[]下角标和值列出来
    ...
    def train(self,batch_size=1024, epochs=1, initial_epoch=0, verbose=1, times=1):
        ...
        hist = self.model.fit_generator(self.batch_it, epochs=epochs, initial_epoch=initial_epoch, steps_per_epoch=self.step_per_epoch,
                                        verbose=verbose)
